<?php
  ...
  $myfile = fopen($_SERVER['DOCUMENT_ROOT'] . '/../transaction_logs/test_late_cancel_first_'.$cleanName."_".strtolower(date('l_F_d_Y_H:m:s')).".txt", "w") or die("Unable to open file!");
          ...
          if($vars['service_provided'] == 14) {
            if($price == $row['price']){
              if($vars['label'] == $row['misc_billing']) {
                ...
                $myfile = fopen($_SERVER['DOCUMENT_ROOT'] . '/../duplicate_checks/test_late_cancel_first_'.$cleanName."_".strtolower(date('l_F_d_Y_H:m:s')).".txt", "w") or die("Unable to open file!");
                foreach($vars as $key => $var) {
                  fwrite($myfile, "Key: " . $key . "  | Value: " . $var . "\n");
                }
                fwrite($myfile, "\n\n");
