<?php
  ...
  $myfile = fopen($_SERVER['DOCUMENT_ROOT'] . '/../transaction_logs/misc_billing_'.$cleanName."_".strtolower(date('l_F_d_Y_H:m:s')).".txt", "w") or die("Unable to open file!");
      ...
      while($row = $result->fetch_assoc()) {
          ...
          if($vars['psychologist'] == $row['psychologist']) {
              ...
              if($curMonth == $dbMonth && $curYear == $dbYear) {
                  ...
                  if($vars['service_provided'] == 18) {
                      if($price == $row['price']){
                          if($vars['label'] == $row['misc_billing']) {
                              $duplicate = true;
                              
                              $myfile = fopen($_SERVER['DOCUMENT_ROOT'] . '/../duplicate_checks/misc_travel_expenses_'.$cleanName."_".strtolower(date('l_F_d_Y_H:m:s')).".txt", "w") or die("Unable to open file!");
                              foreach($vars as $key => $var) {
                                  fwrite($myfile, "Key: " . $key . "  | Value: " . $var . "\n");
                              }
                              fwrite($myfile, "\n\n");
